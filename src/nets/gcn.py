...
class StandGCNEncoder(nn.Module):
    def __init__(self, nfeat, nhid, nembed, dropout,nlayer=2):
        super(StandGCNEncoder, self).__init__()
        self.conv1 = GCNConv(nfeat, nhid, cached= False, normalize=True)
        self.conv2 = GCNConv(nhid, nembed, cached=False, normalize=True)
        self.dropout_p = dropout
    ...
    def forward(self, x, adj, edge_weight=None):
        ...
        return x
class StandGCNClassifier(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout,nlayer=2):
        super(StandGCNClassifier, self).__init__()
        self.conv1 = GCNConv(nfeat, nhid, cached= False, normalize=True)
        self.conv2 = GCNConv(nhid, nclass, cached=False, normalize=True)
...
def create_gcn(nfeat, nhid, nclass, nembed, dropout, nlayer):
    if nlayer == 1:
        model = StandGCN1(nfeat, nhid, nclass, dropout,nlayer)
    elif nlayer == 2:
        model = StandGCN2(nfeat, nhid, nclass, dropout,nlayer)
    elif nlayer == 3:
        model = StandGCNEncoder(nfeat, nhid, nembed, dropout, nlayer)
    elif nlayer == 4:
        model = StandGCNClassifier(nembed, nhid, nclass, dropout, nlayer)
    else:
