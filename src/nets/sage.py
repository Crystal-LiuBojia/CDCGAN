...
class GraphSAGEX(nn.Module):
    ...
    def forward(self, x, adj, edge_weight=None):
        ...
        return x
class StandSAGEEncoder(nn.Module):
    def __init__(self, nfeat, nhid, nembed, dropout,nlayer=2):
        super(StandSAGEEncoder, self).__init__()
        self.conv1 = SAGEConv(nfeat, nhid)
        self.conv2 = SAGEConv(nhid, nembed)
        self.dropout_p = dropout
    ...
    def forward(self, x, adj, edge_weight=None):
        ...
        return x
class StandSAGEClassifier(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout,nlayer=2):
        super(StandSAGEClassifier, self).__init__()
        self.conv1 = SAGEConv(nfeat, nhid)
        self.conv2 = SAGEConv(nhid, nclass)
    ...
    def forward(self, x, adj, edge_weight=None):
        ...
        return x
def create_sage(nfeat, nhid, nclass, nembed, dropout, nlayer):
    if nlayer == 1:
        model = GraphSAGE1(nfeat, nhid, nclass, dropout, nlayer)
    elif nlayer == 2:
        model = GraphSAGE2(nfeat, nhid, nclass, dropout, nlayer)
    elif nlayer == 3:
        model = StandSAGEEncoder(nfeat, nhid, nembed, dropout, nlayer)
    elif nlayer == 4:
        model = StandSAGEClassifier(nembed, nhid, nclass, dropout, nlayer)
    else:
